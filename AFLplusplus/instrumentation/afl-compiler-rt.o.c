...
void __sanitizer_cov_trace_pc_guard__(uint32_t *guard) {
...
static void SetData (QNode *QN, ObjValue *Obj)
{
...
static unsigned CmpData (QNode *Q1, QNode *Q2)
{
  if (Q1->TrcKey == Q2->TrcKey)
  {
    ObjValue *OV1 = (ObjValue *)Q1->Buf;
    ObjValue *OV2 = (ObjValue *)Q2->Buf;
...
static inline void sanitizer_cov_trace_into_queue (uint32_t Key, uint32_t ValLength, uint64_t Value)
{
    ...
    ObjValue Obj;
    Obj.Value = (unsigned)Value;
    
    #if 0
    QNode* QN;
    do
    {
      QN = InQueueKey (Key, (Q_SetData)SetData, &Obj);
    } while (QN == NULL);
    #else
    InQueueKeyOpt (Key, (Q_SetData)SetData, (Q_Cmpata)CmpData, &Obj);
    #endif
...
void __analysis_trace_function_id (uint32_t *guard, uint32_t Key, uint32_t FuncId) {
    if (guard != NULL)
        __sanitizer_cov_trace_pc_guard__ (guard);
        
    sanitizer_cov_trace_into_queue (Key, 4, FuncId);
}

void __analysis_trace_init () {
  ClearQueue ();
}

void __analysis_trace_exit () {
